...
# jtm - Function to run Java test methods on one or multiple test classes
...
jtm() {
  ...
  if [[ -n "$java_test_files" ]]; then
    ...
    echo "Selected Test Classes:"
    echo "$java_test_files"
    echo

    local selected_test_class
    for selected_test_class in "${(@f)java_test_files}"; do
      local test_methods_list=$(rg -o -N "(public|private|protected)?\s*void\s+\K\w+\b\s*\(.*\)" "$selected_test_class")

      if [[ -n "$test_methods_list" ]]; then
        local selected_methods=$(echo "$test_methods_list" | fzf --multi)

        while IFS= read -r method; do
          local trimmed_method=${method%%(*} # Remove the arguments from the method name
          test_methods+="$selected_test_class::$trimmed_method,"
        done <<< "$selected_methods"
      fi
    done
    ...
    echo "Selected Test Methods:"
    echo "$test_methods"
    echo

    if [[ -n "$test_methods" ]]; then
      echo "Running Maven tests for the following methods:"
      echo "$test_methods"
      echo

      # Execute the mvn test command with specific methods
      mvn test -Dtest="$test_classes#$test_methods"
    else
      echo "No test methods selected. Exiting..."
    fi
  fi
}
